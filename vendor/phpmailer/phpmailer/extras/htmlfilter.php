...
function tln_fixurl($attname, &$attvalue, $trans_image_path, $block_external_images)
{
    ...
    /**
     * Replace empty src tags with the blank images.  src is only used
     * for frames, images, and images inputs.  Doing a replace should
...
function tln_body2div($attary, $trans_image_path)
{
    ...
    if (is_array($attary) && sizeof($attary) > 0){
        foreach ($attary as $attname=>$attvalue){
            ...
            switch ($attname){
                case 'background':
                    $styledef .= "background-images: url('$trans_image_path'); ";
                    break;
                case 'bgcolor':
                    $has_bgc_stl = true;
