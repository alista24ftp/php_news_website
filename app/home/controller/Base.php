...
class Base extends Common
{
	...
	protected function _initialize()
    {
		...
		$ym_theme_path=__ROOT__.'/app/home/view/'.$theme.'/';
		$this->assign($site_options);
		$this->assign('ym_theme_path',$ym_theme_path);
		$address='';
		$this->user=array();
		$uid=session('hid');
		if(empty($uid)){
        ...
        $menu_list=Db::name("menu")
            ->where(['menu_l'=>'zh-cn', 'menu_open'=>1, 'menu_type'=>1])
            ->order("listorder","asc")
            ->limit(30)
            ->select();
