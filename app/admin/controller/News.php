...
class News extends Base
{
	...
	public function news_list()
	{
		...
		if ($news_columnid!=''){
			//$ids=get_menu_byid($news_columnid,0,2, 'id');
            $submenus = Db::name('menu')->field('id')->where(['parentid'=>$news_columnid])->select();
            if(!empty($submenus)){
                $ids = array_column($submenus, 'id');
                array_unshift($ids, $news_columnid); // 包括1级栏目自己id
                $map['news_columnid']= array('in',implode(",", $ids));
            }else{
                $map['news_columnid'] = $news_columnid;
            }
